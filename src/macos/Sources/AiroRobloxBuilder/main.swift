...
struct ContentView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack { Text("◈ airobloxbuilder").font(.title2.weight(.semibold)); Spacer(); Text("● ready").foregroundStyle(.green) }.padding(.horizontal, 18).frame(height: 54)
            Divider()
            HStack(spacing: 0) { ProjectPane(); Divider(); ChatPane(messages: $messages, prompt: $prompt); Divider(); AgentPane() }
            Divider()
            HStack { Text("free forever • native • windows first").foregroundStyle(.secondary); Spacer(); Text("v0.1 foundation").foregroundStyle(.secondary) }.padding(.horizontal, 18).frame(height: 62)
        }
    }
}
...
struct ChatPane: View {
    ...
    private func send() {
        ...
        messages.append(value.hasPrefix("/plan") ? "builder: planning is ready for the native core." : "builder: request accepted. routing is ready for the orchestrator.")
        prompt = ""
    }
}
